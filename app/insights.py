...
def hourly_distribution(batch:Dataset)->Dict:
    ...
    return {"hourly_distribution": [json.dumps(dict(hourly_counts))] * len(batch["text"])}
...
def data_insights(ds: Dataset) -> Dict:
    ...
    for func in insight_functions:
        ...
        for column in result.column_names:
            if column not in ds.column_names:  # Identify new columns added by the function
                insights[column] = result[column][0]  # Extract the first value or summary
