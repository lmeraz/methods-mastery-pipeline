...
from app.etl import ETL, DATASET_NAME, DATASET_SPLIT
...
def mock_dataset():
    ...
    """
    data = {"text": ["post1", "post2", "post3"], "created_at": ["2024-12-26T12:00:00", "2024-12-26T13:00:00", "2024-12-26T14:00:00"]}
...
def test_extract(mock_load_dataset, etl_instance, mock_dataset):
    ...
    mock_load_dataset.assert_called_once_with(DATASET_NAME, split=DATASET_SPLIT, cache_dir=etl_instance.cache_path)
    assert isinstance(dataset, Dataset), "The extracted data should be a Dataset instance."
...
def test_transform(mock_data_insights, etl_instance, mock_dataset):
    ...
    assert insights == {"unique_posts": 3}, "The transform method should return the expected insights."
...
def test_has_new_version_no_previous(etl_instance, mock_db_handler):
    ...
    assert etl_instance.has_new_version("1.0") is True, "Should return True when no previous version exists."
...
def test_has_new_version_newer_version(etl_instance, mock_db_handler):
    ...
    assert etl_instance.has_new_version("2.0") is True, "Should return True for a newer version."
...
def test_has_new_version_no_new_version(etl_instance, mock_db_handler):
    ...
    assert etl_instance.has_new_version("1.0") is False, "Should return False when no new version is available."
